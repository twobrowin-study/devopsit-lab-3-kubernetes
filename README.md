...
Minikube является средством локального развёртывания Kubernetes (k8s), предназначенное для установки на индивидуальный компьютер разработчика.
...
* Control Plane - kube-api, etcd, scheduler, controller-manager
...
Следует скачать последнюю версию при помощи команды:
...
После после подготовки кластера, Minikube автоматически настроит утилиту kubectl путём установки стандартной конфигурации в файле `.kube/config`.
...
Созданный нами ранее под был создан в пространстве имён default. Это пространство имён редко используется на практике поскольку k8s кластер предполагает большой масштаб и совместную работу большого числа специалистов.
...
Начнём запуск приложения с подготовки пространства имён. Имя пространства имён установим соответствующим логину пользователя в МГТУ им. Н.Э. Баумана.
...
Для задач настоящей лабораторной все описания будем добавлять в единый yaml файл `k8s.yaml`.
...
* `LoadBalancer` - позволяет присвоить внешний IP адрес сервису, обращение только к нему приведёт к передаче запроса в сервис, используется для захвата стандартных портов.
...
Следует выполнить доступ с использованием ip адреса, присвоенному ноде, т.е. компьютеру, на котором выполняется задание. Этот адрес поможет узнать утилита `ip addr show`.
...
**В отчёт следует занести код полученного сценария обращения к сервису и выводы о последовательности настройки.**

Особенностью применения пространство имён k8s является то, что можно удалить все ресурсы в пространстве имён путём удаления только пространства имён.
...
Для того чтобы обеспечить работу LoadBalancer, который описан в файле `postgres-db.yaml` следует обеспечить туннелирование запросов minikube:
...
Т.е. используя единую ip адрес и доменное имя при помощи описания путей доступа к конкретным микросервисами объединить их, в стиле, как это выполнялось в ЛБ2 с развертывание Nginx.
...
Очень распространённым является использование альтернативной утилиты для работы с Kubernetes API - Helm.
